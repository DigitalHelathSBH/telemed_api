...
const { registerAppointment } = require("../services/telemedCenter.service");
...
function mapToRegisterPayload(row) {
  return {
    hn: row.HN,
    vn: "False",
    ...
    */
    ...
    for (const row of rs.recordset) {
      ...
      try {
        ...
        const payload = mapToRegisterPayload(row);
        const tempData = await registerAppointment(payload);

          if (tempData?.status === "success") {
            const transaction_id =
              tempData?.result?.transaction_id ?? null;
            ...
            await pool.request()
              ...
              .input("APPOINTMENTNO", sql.VarChar, row.APPOINTMENTNO) // หรือ row.APPOINTMENTNO / row.APPOINTMENTNO ตามชื่อฟิลด์ใน select
              .query(sqlUpdate);

            console.log("UPDATED transaction_id:", row.APPOINTMENTNO, transaction_id);

          }else if (tempData?.status === "fail") {

            const message = tempData?.message ?? "unknown error";

            const sqlUpdateFail = `
              UPDATE HNAPPMNT
              SET transaction_id = @message
                  ,TelemedStatus = 'U'
              WHERE APPOINTMENTNO = @APPOINTMENTNO
            `;
            ...
            console.log("UPDATED fail message:", row.APPOINTMENTNO, message);
          }

        console.log("REGISTER RESULT:", tempData);

      } catch (err) {
        console.error("API ERROR APPOINTMENTNO:", row.APPOINTMENTNO);
